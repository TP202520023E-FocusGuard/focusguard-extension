<template>
  <div class="popup">
    <header class="popup__hero">
      <Tag class="brand-pill" value="FocusGuard" rounded severity="contrast" />
      <h1 class="popup__title">{{ headline }}</h1>
      <p class="popup__subtitle">{{ helperText }}</p>
    </header>

    <section class="popup__context">
      <InfoBlock
        icon="🌐"
        :title="domain"
        :description="contextDescription"
      />
      <Message v-if="errorMessage && !isLoading" severity="warn" :closable="false">
        {{ errorMessage }}
      </Message>
    </section>

    <section class="popup__stats">
      <StatHighlight
        icon="🎯"
        label="Categoría"
        :value="category"
    ...
    <section class="popup__actions">
      <Button
        ...
        :label="manualButtonDisabled ? 'Esperando datos...' : manualButtonLabel"
...
const getPageMetadata = async (tabId: number) => {
...
const resolveLeisureClassification = async (tabId: number, categoryValue: string) => {
  if (categoryValue !== 'Doble filo') {
    return 'No ocio';
  }
...
const isBrowserContext = Boolean(chromeApi?.tabs?.query) && Boolean(chromeApi?.scripting);
...
const contextDescription = computed(() => {
  if (isLoading.value) {
    return 'Analizando la pestaña activa...';
  }
  if (errorMessage.value) {
    return 'No pudimos obtener detalles de la pestaña activa.';
  }
  return 'Este es el sitio que estás visitando en este momento.';
});
