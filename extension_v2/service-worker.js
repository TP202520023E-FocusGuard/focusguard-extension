...
async function shouldLaunchIntervention(tabId, interventions_activated) {
  ...
  if (wasUnlocked) {
    ...
    /*
        console.log("shouldLaunchIntervention was unlocked");

        const { accum_leisure_int = 0, leisure_start_int = null } =
            await chrome.storage.local.get(["accum_leisure_int", "leisure_start_int"]);

        // Calculamos el tiempo total real (acumulado estático + tiempo de la sesión actual si existe)
        let totalLeisureTimeInt = accum_leisure_int;
        if (leisure_start_int) {
          const currentSessionSeconds = Math.floor((Date.now() - leisure_start_int) / 1000);
          totalLeisureTimeInt += currentSessionSeconds;
        }

        console.log(`Tiempo total en ocio desde la última intervención: ${totalLeisureTimeInt}s / Objetivo: ${TIME_BETWEEN_INTERVENTIONS / 1000}s`);

        // Evaluamos usando el tiempo real calculado
        if (totalLeisureTimeInt >= TIME_BETWEEN_INTERVENTIONS / 1000) {
          console.log("INTERVENCIÓN RECURRENTE LANZADA");
          await injectIntervention(tabId);
        }
    */
  } else {
...
async function finishOcioSession() {
  let { leisure_start = null, accumulated_leisure_time, assigned_rest_time = 0 } = await chrome.storage.local.get(["leisure_start", "accumulated_leisure_time", "assigned_rest_time"]);
  ...
  if (leisure_start) {// Si existe un registro de Ocio
    ...
    const interventions_activated = new_accumulated >= (assigned_rest_time * 60);
